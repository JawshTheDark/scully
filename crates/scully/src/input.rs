...
/// characters). Returns `(start, new_text, new_cursor)` where `start` is the
/// completion anchor to reuse for cycling.
///
/// The IRC conventions: completing at the start of the line appends `": "`
/// (addressing someone), mid-line appends a single space.
pub fn complete(
    text: &str,
    cursor: usize,
    start: usize,
    ...
    let insert = if start == 0 { format!("{nick}: ") } else { format!("{nick} ") };
...
mod tests {
    ...
    fn completing_at_line_start_addresses_the_nick() {
        ...
        let (text, cursor) = complete("rep", 3, start, "reprouser");
    ...
    fn completing_mid_line_appends_a_space() {
        ...
        let (new, cursor) = complete(text, 10, start, "reprouser");
    ...
    fn completion_preserves_the_tail_after_the_cursor() {
        ...
        let (new, _) = complete(text, 3, start, "reprouser");
    ...
    fn cycling_reuses_the_anchor() {
        ...
        let (text, cursor) = complete("alice: ", 7, 0, "alicia");
    ...
    fn unicode_prefixes_complete_without_panicking() {
        ...
        assert_eq!(prefix, "bj");
        let (new, _) = complete(text, text.chars().count(), start, "bjourne");
